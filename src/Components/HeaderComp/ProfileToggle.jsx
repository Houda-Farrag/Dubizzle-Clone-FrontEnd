...
function ProfileToggle({setGoogle, setFacebook, setData , profile}) {
    ...
    return (<>
        <div onClick={toggleProfileList} className="relative mt-3 ms-2">
            <p className="font-bold text-sm hover:cursor-pointer flex" ><span className='bg-red-50 text-red-600 w-8 h-8 flex justify-center items-center  rounded-full'> {profile?.profile.name.slice(0,2)} </span>
            ...
            {showProfileList &&
                <div className="profile absolute right-0 bg-white text-black shadow-2xl overflow-y-scroll  top-12 rounded-sm w-vw-21/100  h-vh-80/100">
                    <div className='border-b pb-4 border-gray-300'>
                        <div className="name flex p-4 gap-4 items-center  relative text-sm" ><span className='bg-red-50 text-2xl font-bold text-red-600 w-16 h-16 flex justify-center items-center  rounded-full'>{profile?.profile.name.slice(0,2)}</span>
                            <div>
                                <p className='text-md'>Hello,</p>
                                <p className='text-xl font-bold'>{profile?.profile.name}</p>
                            </div>
                        </div>
                    ...
                    <div className='border-b border-gray-300'>
                        ...
                        <div className='flex items-center ps-4 h-11 hover:bg-slate-100 cursor-pointer'>
                            ...
                            <p className='ms-3' onClick={()=>{navigate('/favorite')}}>Favourite</p>
                        </div>
                        <div className='flex items-center ps-4 h-11 hover:bg-slate-100 cursor-pointer'>
                            <div className='w-6 flex justify-center'>
                    ...
                    <div className='flex items-center ps-4 h-11 hover:bg-slate-100 cursor-pointer'>
                        ...
                        <p className='ms-3 cursor-pointer' onClick={handleLogout}>Logout</p>
                    </div>
                </div>
            }
